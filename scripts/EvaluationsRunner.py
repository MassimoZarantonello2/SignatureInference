...
class EvaluationsRunner:
    def __init__(
        ...
        self.problem_type = problem_type if problem_type is None else [
            'binary'] * len(labels)
        self.labels = None
    ...
    def compute_evaluations_metrics(self, evaluation_df, sample):
        ...
        lc = LogClass(sample)
        lc.log(
            f'For sample {sample} the train and test dataframes are created')
        try:
    ...
    def threaded_evaluation(self, run, sample, output_dict, tissues, lock):
        ...
        if tissues == 'feature':
            tissues_df = pd.read_csv(
                self.tissues_path).drop(
                columns=['Cohort'])
            run_sample_df = pd.merge(
                run_sample_df, tissues_df, on='Unnamed: 0')
    ...
    def run_evaluations(self, tissues):
        ...
        for run in self.run_values:
            ...
            for sample in self.sampling_values:
                ...
                } or all_evaluations_df[run_index][sample_index] is None:
                    print(f'Running run {run} and sample {sample}')
                    lc = LogClass(sample)
                    lc.log(f'Starting run {run} and sample {sample}')
                    t = threading.Thread(
                        target=self.threaded_evaluation, args=(
                            run, sample, output_dict, tissues, lock))
            ...
            for key in output_dict:
                all_evaluations_df[run_index]['sampling_' +
                                              key] = output_dict[key]

            if signature_inference_thread.__len__() != 0:
                json.dump(
                    all_evaluations_df, open(
                        self.save_evaluation_path, 'w'))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Numero specifico di run da eseguire')
